import { addNavMenu, getNavList, setNavMenu } from '@/services/api';
...
const AccessPage: React.FC = () => {
  ...
  const { data, run } = useRequest(() => getNavList(pageInfoRef.current), {
    manual: true,
  });
  ...
  const handleOnModalConfrirm = async (value: any) => {
    ...
    if (modalType === mtype.CREATE) {
      result = await addNavMenu(value);
    } else {
      result = await setNavMenu(value);
    }
